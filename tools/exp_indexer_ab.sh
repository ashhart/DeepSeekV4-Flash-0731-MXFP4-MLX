...
#   B = batched single-eval variant (marker ~/.omlx/ds4_indexer_pad)
#
...
set -u
LOG=/Users/ash/ds4/exp_indexer_ab.log
...
cleanup() {
    ...
    # Production is now spec+consist+router ON: restore that, not the old state.
    rm -f "$HOME/.omlx/ds4_spec_timing" "$HOME/.omlx/ds4_indexer_pad" \
...
# Full promoted stack ON in both arms; only the indexer marker toggles.
...
run_one() {  # $1 = label, $2 = batched(0/1)
    if [ "$2" = "1" ]; then touch "$HOME/.omlx/ds4_indexer_pad"; else rm -f "$HOME/.omlx/ds4_indexer_pad"; fi
...
log "run A1 (indexer fallback)";   run_one A1 0
check_valid
log "run B1 (indexer pad)";   run_one B1 1
if ! tail -n +"$LOG_START" "$SRVLOG" | grep -q "indexer pad ENGAGED"; then
    log "VOID: fused router never engaged in B1 -- lever not wired"
    exit 1
fi
log "run A2 (indexer fallback)";   run_one A2 0
log "run B2 (indexer pad)";   run_one B2 1
