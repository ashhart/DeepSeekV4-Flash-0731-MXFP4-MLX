#!/bin/sh
# Hyper-connection pre-op fusion A/B (FINDINGS S33/S35).
#
...
set -u
LOG=/Users/ash/ds4/exp_hc_fuse_ab.log
...
run_one() {  # $1 = label, $2 = hc_fuse on(1)/off(0)
    if [ "$2" = "1" ]; then touch "$HOME/.omlx/ds4_hc_fuse"; else rm -f "$HOME/.omlx/ds4_hc_fuse"; fi
...
for pass_no in 1 2; do
    for arm in on off; do
        [ "$arm" = "on" ] && v=1 || v=0
        log "run ${arm}-p${pass_no} (layer_async $arm)"
        run_one "${arm}-p${pass_no}" "$v"
        check_valid
        if [ "$arm" = "on" ] && ! grep -q "hc fuse ENGAGED" "$SRVLOG"; then
            log "VOID: hc fuse never engaged -- lever not wired"
            exit 1
        fi
    done
done
