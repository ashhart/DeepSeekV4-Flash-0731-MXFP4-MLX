...
set -u
LOG=/Users/ash/ds4/exp_layer_async_ab.log
...
run_one() {  # $1 = label, $2 = layer_async on(1)/off(0)
    if [ "$2" = "1" ]; then
        printf 'decode=off\nmulti=2\n' > "$HOME/.omlx/ds4_layer_async"
    else
        rm -f "$HOME/.omlx/ds4_layer_async"
    fi
