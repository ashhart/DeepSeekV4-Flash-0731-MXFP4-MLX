...
#   B = batched single-eval variant (marker ~/.omlx/ds4_consist_batched)
#
...
set -u
LOG=/Users/ash/ds4/exp_commit_gap.log
...
cleanup() {
    ...
    rm -f "$HOME/.omlx/ds4_spec_timing" "$HOME/.omlx/ds4_consist_batched" \
...
run_one() {  # $1 = label, $2 = batched(0/1)
    if [ "$2" = "1" ]; then touch "$HOME/.omlx/ds4_consist_batched"; else rm -f "$HOME/.omlx/ds4_consist_batched"; fi
...
log "run A1 (stock consistency)";   run_one A1 0
check_valid
log "run B1 (batched consistency)"; run_one B1 1
log "run A2 (stock consistency)";   run_one A2 0
log "run B2 (batched consistency)"; run_one B2 1
